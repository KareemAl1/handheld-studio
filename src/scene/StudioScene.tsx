  ...
  useEffect(() => {
    ...
    window.__HS_STUDIO__ = {
      snapshot: () => ({
        ...
        camera: camera.position.toArray(), dpr: gl.getPixelRatio(),
...
export default function StudioScene({ config, view, onReady, onOrbit }: {
  config: Configuration; view: ViewRequest; onReady: () => void; onOrbit: () => void;
}) {
  ...
  return <Canvas frameloop="demand" dpr={[1, 1.75]} camera={{ fov: 32, near: 0.1, far: 60, position: [3.5, 3, 10] }}
    ...
    <Suspense fallback={<Html center><div className="asset-loading" role="status">Preparing your handheld…</div></Html>}>
      ...
      <ConsoleModel config={config} onReady={onReady} />
      <ContactShadows position={[0, -1.92, 0]} opacity={0.38} scale={18} blur={2.6} far={5} resolution={512} frames={1} color="#414338" />
    </Suspense>
    <CameraControls view={view} onOrbit={onOrbit} />
